...
-- ----------------------------
-- Table structure for cms_question_examination_answer_result
-- ----------------------------
DROP TABLE IF EXISTS `cms_question_examination_answer_result`;
CREATE TABLE `cms_question_examination_answer_result`
(
    `examination_answer_result_id` int(11) unsigned NOT NULL AUTO_INCREMENT,
    `examination_answer_id`        int(11) DEFAULT NULL,
    `examination_id`               int(11) DEFAULT NULL,
    `item_id`                      int(11) DEFAULT NULL COMMENT '题目id',
    `option_values`                varchar(1024) DEFAULT '' COMMENT '回答内容',
    `is_fill`                      tinyint(1) DEFAULT '0' COMMENT '是否填空题',
    `status`                       tinyint(1) DEFAULT '0' COMMENT '0 未提交，1 已完成、2未改判（填空题）',
    `is_answer_correct`            int(2) DEFAULT '0' COMMENT '选择题回答是否正确(0不正确，1正确)',
    `score`                        int(11) DEFAULT '0' COMMENT '等分',
...
(
    ...
    `examination_id` int(11) DEFAULT '0' COMMENT '关联问卷id',
